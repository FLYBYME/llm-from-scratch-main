...
├── src/
│   ├── model.ts           # GPT architecture (Part 2)
│   ├── train.ts           # Training loop (Part 3)
│   ├── generate.ts        # Text generation (Part 4)
...
The default config trains a small GPT model on Shakespeare. You'll see:
...
Generation is currently integrated into `src/main.ts`. After training completes, it will automatically generate a sample prompt.
