...
No file to create here — the tokenizer is built directly into `src/train.ts` (Part 3). This part explains how it works so you understand what you're writing later.
...
```typescript
const text = fs.readFileSync("../data/shakespeare.txt", "utf-8");
const chars = [...new Set(text)].sort();
const vocabSize = chars.length; // 65 for Shakespeare

const stoi = Object.fromEntries(chars.map((c, i) => [c, i])); // string to int
const itos = Object.fromEntries(chars.map((c, i) => [i, c])); // int to string

function encode(s: string): number[] {
    return [...s].map(c => stoi[c]);
}
...
```typescript
import { Tokenizer } from "gpt-tokenizer"; // or similar library
const tokens = Tokenizer.encode("Hello, world!");  // [15496, 11, 995, 0]
const text = Tokenizer.decode(tokens);             // "Hello, world!"
```
...
1. **The embedding table** — `tf.layers.embedding({ inputDim: vocabSize, outputDim: n_embd })` maps each token ID to a learned vector. With vocab_size=65, this is tiny (65 × 384 = 24,960 parameters). With GPT-2's vocab of 50,257, it's 50,257 × 384 = 19.3M parameters — nearly half the model.

2. **The output layer** — `tf.layers.dense({ units: vocabSize })` produces a probability over all possible next tokens. With 65 tokens, the model chooses from 65 options. With 50,257 tokens, it must spread its predictions across 50,257 classes.
